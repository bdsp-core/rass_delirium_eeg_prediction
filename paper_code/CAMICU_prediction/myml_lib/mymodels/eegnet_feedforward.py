...
#from eegnet_recurrent import EEGNet_RCNN
...
class EEGNet_CNN(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        if ff_input:
            ...
            #Hspec, _ = self.spec_enc(x)
            #Hspec = Hspec.view(N,-1)
            ...
            #x2 = x.view(-1, x.shape[2], x.shape[3])
            #Hspec, _ = self.spec_enc(x2)
            #Hspec = Hspec.view(N, -1, Hspec.shape[1]*Hspec.shape[2])
...
class EEGNet_CNN_CAMICU(nn.Module):
    """
    """
    def __init__(self, filter_num=8):
        super(EEGNet_CNN_CAMICU, self).__init__()
        self.dropout = None##############################TODO 0.1
        self.cnn_filter_num = filter_num
        self.cnn_filter_len = 9
        self.n_channel = 2
        self.subsampling = 4
        self.padding = self.cnn_filter_len//2
        self.momentum = 0.1
        ...
        self.first_layer = nn.Conv2d(self.n_channel, self.cnn_filter_num, (self.cnn_filter_len,1), stride=1, padding=(self.padding,0), bias=False)
        #(F, 3000)
        self.resblock0 = ResBlock2d(self.cnn_filter_num, self.cnn_filter_num, self.cnn_filter_len,
                self.dropout, self.subsampling, self.momentum, bn_affine=self.bn_affine, maxpool_padding=1, act='leaky', residual=False)
        #(F, 750)
        self.resblock1 = ResBlock2d(self.cnn_filter_num, self.cnn_filter_num, self.cnn_filter_len,
                self.dropout, 1, self.momentum, bn_affine=self.bn_affine, act='leaky', residual=False)
        #(F, 750)
        self.resblock2 = ResBlock2d(self.cnn_filter_num, self.cnn_filter_num, self.cnn_filter_len,
                self.dropout, self.subsampling, self.momentum, bn_affine=self.bn_affine, act='leaky', residual=False)
        #(F, 188)
        self.resblock3 = ResBlock2d(self.cnn_filter_num, self.cnn_filter_num*2, self.cnn_filter_len,
                self.dropout, 1, self.momentum, bn_affine=self.bn_affine, act='leaky', residual=False)
        #(2*F, 188)
        self.resblock4 = ResBlock2d(self.cnn_filter_num*2, self.cnn_filter_num*2, self.cnn_filter_len,
                self.dropout, 8, self.momentum, bn_affine=self.bn_affine, act='leaky', residual=False)
        #(2*F, 24)
        self.resblock5 = ResBlock2d(self.cnn_filter_num*2, self.cnn_filter_num*2, self.cnn_filter_len,
                self.dropout, 1, self.momentum, bn_affine=self.bn_affine, act='leaky', residual=False)
        #(2*F, 24)
        self.resblock6 = ResBlock2d(self.cnn_filter_num*2, self.cnn_filter_num*3, self.cnn_filter_len,
                self.dropout, 3, self.momentum, bn_affine=self.bn_affine, act='leaky', residual=False)
        #(3*F, 8)
        self.resblock7 = ResBlock2d(self.cnn_filter_num*3, self.cnn_filter_num*3, self.cnn_filter_len,
                self.dropout, self.subsampling, self.momentum, bn_affine=self.bn_affine, act='leaky', residual=False)
        #(3*F, 2)
        """
        #(nch, 250)
        ...
        #(F, 63)
        self.resblock2 = ResBlock2d(self.cnn_filter_num, self.cnn_filter_num, self.cnn_filter_len,
                self.dropout, self.subsampling, self.momentum, maxpool_padding=1, bn_affine=self.bn_affine, act='leaky', residual=False)
        #(F, 16)
        self.resblock3 = ResBlock2d(self.cnn_filter_num, self.cnn_filter_num*2, self.cnn_filter_len,
                self.dropout, 1, self.momentum, bn_affine=self.bn_affine, act='leaky', residual=False)
        #(2*F, 16)
        ...
        #(2*F, 4)
        self.resblock6 = ResBlock2d(self.cnn_filter_num*2, self.cnn_filter_num*2, self.cnn_filter_len,
                self.dropout, 2, self.momentum, bn_affine=self.bn_affine, act='leaky', residual=False)
        #(3*F, 2)
        self.resblock7 = ResBlock2d(self.cnn_filter_num*2, self.cnn_filter_num*3, self.cnn_filter_len,
                self.dropout, 1, self.momentum, bn_affine=self.bn_affine, act='leaky', residual=False)
        #(3*F, 2)
        ...
        self.Hdim = self.cnn_filter_num*3*2
        self.output_layer = nn.Linear(self.Hdim, 1)
        self.output_act = None
        self.output_act2 = nn.Sigmoid()
    ...
    def forward(self, x, **kwargs):
        ...
        if ff_input:
            ...
            output = self.output_layer(Hcnn.view(-1, Hcnn.shape[-1]))
            output = output.view(N, -1, output.shape[-1])
            
        if self.training:
            if output is not None and hasattr(self, 'output_act') and self.output_act is not None:
                output = self.output_act(output)
        else:
            if output is not None and hasattr(self, 'output_act2') and self.output_act2 is not None:
                output = self.output_act2(output)
