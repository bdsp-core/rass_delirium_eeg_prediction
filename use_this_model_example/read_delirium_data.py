...
def read_delirium_mat(fn, channel_names=None, with_data=True):
    ...
    if channel_names is not None:
        channel_names = list(map(lambda x:x.lower(), channel_names))
    labels = list(map(lambda x:x.tolist()[0], res['labels'].flatten()))
    Fs = round(res['Fs'][0,0]*1000)/1000.
         
    if len(labels)!=5:
        raise TypeError('Not 5 channels in %s'%fn)
    if channel_names is not None and np.any(list(map(lambda i:channel_names[i] not in labels[i].lower(), range(len(labels))))):
    ...
    if with_data:
        ...
        if data.shape[0]!=5:
            raise TypeError('Not 5 channels in %s'%fn)
        myres['data'] = data[[0,1,3,4]]  # not use the referential electrode
        data_length = data.shape[1]
    else:
        try:
            data_length = list(filter(lambda x:x[0]=='data',sio.whosmat(fn))[0][1][1])
        except Exception:
